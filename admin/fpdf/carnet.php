...
class PDF extends FPDF {
    function RoundedRect($x, $y, $w, $h, $r, $style = '') {
        ...
        $this->_Arc($x+$w-$r, $y, $x+$w, $y, $x+$w, $y+$r, $MyArc);
        $this->_out(sprintf('%.2F %.2F l', ($x+$w)*$k, ($hp-$y-$h+$r)*$k ));
        $this->_Arc($x+$w, $y+$h-$r, $x+$w, $y+$h, $x+$w-$r, $y+$h, $MyArc);
        $this->_out(sprintf('%.2F %.2F l', ($x+$r)*$k, ($hp-$y-$h)*$k ));
        $this->_Arc($x+$r, $y+$h, $x, $y+$h, $x, $y+$h-$r, $MyArc);
        $this->_out(sprintf('%.2F %.2F l', $x*$k, ($hp-$y-$r)*$k ));
        $this->_Arc($x, $y+$r, $x, $y, $x+$r, $y, $MyArc);
        $this->_out($op);
    }

    function _Arc($x1, $y1, $x2, $y2, $x3, $y3, $MyArc) {
        $h = $this->h;
        $this->_out(sprintf('%.2F %.2F %.2F %.2F %.2F %.2F c',
            ($x1+$MyArc*($x2-$x1))*$this->k,
            ($h-($y1+$MyArc*($y2-$y1)))*$this->k,
            ($x3+$MyArc*($x2-$x3))*$this->k,
            ($h-($y3+$MyArc*($y2-$y3)))*$this->k,
            $x3*$this->k,
            ($h-$y3)*$this->k));
    }
}
...
// Crear PDF
$pdf = new PDF('L', 'mm', [85.6, 54]);
$pdf->AddPage();
$pdf->SetFillColor(0, 128, 0); // Verde
$pdf->RoundedRect(25, 2, 56, 50, 5, 'F'); // Rectángulo con esquinas redondeadas
$pdf->Output();
...
// $pdf->Image('https://ikusa.net/images/banks/bizum.png', 165, $lineY + 10, 30); // Posición debajo del texto
