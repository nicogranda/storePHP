...
$pdf->SetSubject('Proforma Invoice Details');
$pdf->SetKeywords('TCPDF, PDF, invoice, proforma');

// set default header data
$pdf->SetHeaderData('', PDF_HEADER_LOGO_WIDTH, 'Proforma Invoice', 'Request: '.$proforma_invoice_id , array(0,64,255), array(0,64,128));
...
$logoPath = '../../../../images/logo.png'; // Ajusta esto a la ruta de tu logo
...
$total = 0;
//$proforma_invoice_id = 102;// Obtén el ID de la proforma invoice desde la URL
    ...
    while ($row = $proforma_invoices_details->fetch_assoc()) {
        ...
        if ($products = $mysqli->query("SELECT name, unit FROM products WHERE id = '$product_id'")) {
            ...
            */
            ...
            /*
            $pdf->Cell(70, 10, 'Error retrieving product', 1);
            $pdf->Cell(20, 10, $quantity, 1, 0, 'R');
            $pdf->Cell(30, 10, number_format($unit_value, 2), 1, 0, 'R');
            $pdf->Cell(20, 10, 'N/A', 1, 0, 'R');
            $pdf->Cell(20, 10, number_format($discount, 2), 1, 0, 'R');
            $pdf->Cell(30, 10, number_format($balance, 2), 1, 1, 'R');
            */
        }
