...
$invoices = $mysqli->query("SELECT * FROM invoices WHERE id = '$invoice_id'");
if ($invoices && $row = $invoices->fetch_assoc()) {
...
$pdf->Cell(30, 6, utf8_decode('Invoice'), 0, 1, 'L');
...
//Number
$formatted_quote_id = str_pad($invoice_id, 9, '0', STR_PAD_LEFT);
...
$invoices_details = $mysqli->query("SELECT * FROM invoices_details WHERE invoice_id = '$invoice_id'");
while ($row = $invoices_details->fetch_assoc()) {
    ...
    while ($product = $products->fetch_assoc()) {
        $pdf->Cell(50, 5, utf8_decode($product["name"]), '');
...
$pdf->Cell(165, 8, utf8_decode('Sales tax not applicable'), '', 0, 'L');
